...
def overlay_ann(image, box, label, score, alpha=0.5):
    ...
    #FONT_HERSHEY_SIMPLEX  0.3字体大小   2粗细
    cv2.putText(
        image,
        # "{}: {:.3f}".format(label, score),
        "{}".format(label),
        (int(box[0] / 1.5), int((box[1] + box[3]) / 2)),
        cv2.FONT_HERSHEY_SIMPLEX,
        1, (0, 0, 0), 2
    )
