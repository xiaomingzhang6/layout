...
MODEL_PATH =r'E:\python\MaskRcnn\maskrcnn_table\T\my_model_table.pth'
...
def site_table(box,images):
    ...
    return box
#切割表格   使用绝对路径
def crop_table(box, images, idx, i):
    #保存照片路径
    path = r'E:\python\MaskRcnn\maskrcnn_table\output\{0}_{1}.png'.format(i, idx)
    # 获取坐标
    x_1 = box[0]
    y_1 = box[1]
    x_2 = box[2]
...
def move_img(i, count):
    #保存切割照片的目的路径
    dst_path = r'C:\Users\张晓明\Desktop\crop_table\{}'.format(i)
    #保存输出照片的目的路径
    output_path = r'C:\Users\张晓明\Desktop\output_table' + '\\' + (str(i).zfill(7)) + '.png'
    #保存根路径
    dst_output_path = r'E:\python\MaskRcnn\maskrcnn_table\output\delete_table.png'
    #创建文件夹
    os.mkdir(dst_path)
    '''把布局分析delete_table复制到文件夹'''
    copy(dst_output_path, output_path)
    for j in range(count):
        src_path = r'E:\python\MaskRcnn\maskrcnn_table\output\{0}_{1}.png'.format(i, j)
        copy(src_path, dst_path)

def table_main(img_path, i):
    num_classes = 2
    ...
    for pred in prediction:
        for idx, mask in enumerate(pred['masks']):
            if pred['scores'][idx].item() < 0.8:       #得分小于0.8 不要
                continue
            ...
            #扩大坐标
            box = site_table(box,images)
            #裁剪坐标
            crop_table(box, images, idx, i)
    ...
    cv2.imwrite(r'E:\python\MaskRcnn\maskrcnn_table\output\delete_table.png', image)
    move_img(i, count)
