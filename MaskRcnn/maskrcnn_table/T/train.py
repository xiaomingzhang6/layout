...
def main():
    ...
    dataset = torch.utils.data.Subset(dataset, indices[0:82])
    print("========",len(indices[0:82]),indices[0:82])

    dataset_test = torch.utils.data.Subset(dataset_test, indices[70:82])
    print("========", len(indices[70:82]), indices[70:82])
    ...
    torch.save(model.state_dict(), "my_model_table1.pth")
